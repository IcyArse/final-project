...
# UI libraries
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QPushButton, QVBoxLayout, QHBoxLayout, QSizePolicy, QCheckBox
...
class StarrySky(QWidget):
        def __init__(self):
                ...
                self.downloadCheckBox = QCheckBox(text="Download star image on generation")
                ...
                layout.addWidget(self.downloadCheckBox)
        ...
        def main_app(self):
                try:
                        ...
                        if self.downloadCheckBox.isChecked():
                                try:
                                        self.figure.savefig(f"Starmaps\\{self.location_input.text()}\\{self.year_input.text()}.png", dpi=600, transparent=True)
                                except:
                                        folder_path = Path(f"Starmaps\\{self.location_input.text()}")
                                        folder_path.mkdir(parents=True, exist_ok=True)
                                        self.figure.savefig(f"Starmaps\\{self.location_input.text()}\\{self.year_input.text()}-{self.month_input.text()}-{self.date_input.text()}.png", dpi=600, transparent=True)
